...
- **2026-07-24** — Runner self-hosted do Genesis Lab: `actions/setup-java` instala e seleciona o JDK, mas não disponibiliza o executável `mvn`. O Maven Wrapper no modo `only-script` também não é portátil entre hosts: quando `unzip` não existe, o script troca silenciosamente a distribuição de `.zip` para `.tar.gz`, mas conserva o SHA-256 configurado para o ZIP e a validação falha. Jobs Maven não devem depender de Maven ou `unzip` globais; o backend usa o wrapper no modo `bin`, versiona o JAR oficial com checksum próprio, fixa o checksum da distribuição e executa `./mvnw`. O PostgreSQL de testes permanece efêmero e isolado por service container.
- **2026-07-24** — Isolamento de testes web com JWT: um `@WebMvcTest` que envia um Bearer token textual sem substituir o `JwtDecoder` aciona a descoberta OpenID do `issuer-uri` e passa a depender de Keycloak acessível. Para testar token inválido e resposta 401, declare `JwtDecoder` com `@MockitoBean` e faça o mock lançar `BadJwtException`; testes slice não devem consultar infraestrutura externa. O uso de `.with(jwt())` continua adequado para cenários autenticados porque injeta a autenticação diretamente.
- **2026-07-24** — Runtime, portas e e-mail no job Bruno: cada job de um runner self-hosted começa com o runtime disponível no host; o `setup-node` de outro job não é herdado. O job Bruno deve selecionar Node 22 explicitamente, verificar `crypto.subtle` e usar uma versão fixada do CLI. Serviços de CI não devem disputar portas dos ambientes implantado ou local: backend e MailHog publicam apenas em `127.0.0.1` com portas efêmeras descobertas pelo workflow, e o Keycloak usa a porta dedicada 18282. O workflow injeta `baseUrl`, `mailhogBaseUrl` e `keycloakIssuer` no Bruno. Como o envio SMTP é assíncrono, coleções que capturam tokens no MailHog devem aguardar por polling limitado a mensagem do destinatário esperado; uma consulta única imediatamente após a API de convite pode receber `items: []` e gerar falhas em cascata.
- **2026-07-24** — Configuração do hardening no deploy: `docker compose --env-file` não disponibiliza variáveis para passos anteriores do GitHub Actions. A URL-base pública do Keycloak usada pelo `kcadm` deve ser cadastrada como Repository Variable `KEYCLOAK_URL` (sem `/realms/...`); credenciais administrativas e SMTP permanecem em GitHub Actions Secrets. O arquivo operacional externo também registra a URL para execução manual, mas não substitui a variável do workflow.
