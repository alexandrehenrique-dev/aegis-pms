...
- [ ] `bruno/` tem uma pasta numerada por etapa (03 a 23), a pasta `00-auth` autentica e captura `{{token}}` automaticamente para os 5 usuários de teste, e `npx @usebruno/cli run --env local`, a partir de `bruno/`, roda todo o fluxo (login → CRUD de cada domínio) sem editar nenhum request manualmente.
