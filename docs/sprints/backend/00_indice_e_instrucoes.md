...
Todas as outras etapas deste plano (`01`–`26`) são pensadas para você executar comigo (Claude/Cowork), em modo agent, neste mesmo repositório.
...
3. Peça ao GPT para gerar os arquivos/código pedidos na etapa, aplique-os no repositório, valide — a partir da etapa 03, isso significa pedir ao GPT os arquivos `.bru` novos/alterados da pasta numerada desta etapa em `bruno/` (collection Bruno cumulativa, ver `00_padrao_qualidade_e_arquitetura.md`, Seção 11), salvá-los em `bruno/<NN-nome-da-etapa>/` (versionado normalmente no git, como qualquer arquivo de código) e validar a collection inteira via `npx @usebruno/cli run --env local`, a partir de `bruno/`. Antes de comitar, peça também o **`SPRINT-RESULTADO.md` completo e atualizado** (arquivo inteiro, nunca um diff — mesmo princípio da collection) com a entrada desta etapa (template fixo na Seção 12.2 do padrão de qualidade) e salve-o em `docs/sprints/backend/SPRINT-RESULTADO.md` — só então comite (cada etapa já traz a mensagem de commit sugerida, que passa a incluir este arquivo também).
4. Ao terminar a última etapa (`30_...md`), faça merge da branch em `develop`:
   ```bash
...
- O frontend **não é Angular** — é React (ver `docs/adr/ADR-0011-frontend-framework-react.md`). Onde `implementation/001` menciona Angular (Features 017/018), a etapa `19` desta pasta substitui isso por "buildar e servir o React existente em `frontend/`". Não recrie um frontend Angular.
- Os payloads das etapas 10-18 e 22 (domínios de produto) **não são inventados nesta sprint** — são os mesmos contratos TypeScript que `frontend/src/domains/*/contracts/` já define hoje (Sprint 09 do frontend, fora do GPT), mais os achados da Sprint 11 (domínio `pages`, preview leve de nó do grafo, `summary`/`difficultyLevel` em `Content`). Se o GPT sugerir um shape diferente do descrito na etapa, prevaleça o shape da etapa/trace report, não a sugestão do GPT.
- Toda etapa precisa passar pela validação descrita antes de seguir para a próxima.
- **A partir da etapa 03**: validação é entregue como collection Bruno cumulativa (`bruno/`, um arquivo `.bru` por request), não só `curl` manual — autenticação via Keycloak automática (pasta `00-auth`, token capturado por script e herdado por toda a collection). Detalhe completo: `00_padrao_qualidade_e_arquitetura.md`, Seção 11.
...
| Arquivo | Cobre | Conteúdo |
|---|---|---|
| `19_read_model_forms_submit_publico_e_seguranca.md` | Retrofit etapa 13 + gap etapa 17 | **[INSERIDA — auditoria de pacotes vazios e retrofits]** Read model de forms (`responseCount`/`lastActivityAt` via `SubmissionReceivedEvent`); endpoint público `POST .../forms/{formId}/submit` (sem auth, com rate limiting); endpoint `GET/PUT /settings/security` (`ProductSecuritySettings`); limpeza de pacotes vazios (`seo`, `contract`, `config`, `integration`) e encoding Maven UTF-8 |
| `20_build_frontend_react_no_backend.md` | 017, 018 (substituídas) | Build do React (não Angular) servido pelo Spring Boot |
| `21_dockerfile_e_compose_completo.md` | 019, 020 | Dockerfile do backend + Docker Compose completo (inclui MailHog da etapa 06) |
| `22_seed_inicial_e_grafo.md` | 021, 022 | Seed de tenants/produtos/usuários + seed do Knowledge Graph |
| `23_dominio_pages_secoes_e_blocos.md` | Trace, Seção D.1 (Sprint 11 do frontend) | Domínio `pages`: páginas compostas por seções/blocos tipados (hero, card-list, gallery, contact...) — pré-requisito: etapas 11, 12 e 13; pode ser feita a qualquer momento depois delas, numerada por último por ter sido adicionada depois |
| `24_openapi_testes_e_checklist_final.md` | 023, 024, 025 | Swagger, testes mínimos, checklist final do servidor |

### Domínios adicionais (surgidos depois do checklist final)

| Arquivo | Cobre | Conteúdo |
|---|---|---|
| `25_dominio_notification.md` | Sprint 14 do frontend (`docs/sprints/frontend/14_onboarding_real_e_sistema_de_notificacoes.md`) | Domínio `notification`: onboarding real + notificações direcionadas do Super Admin, com fan-out por destinatário — adicionada **depois** do checklist final (etapa 24) porque surgiu depois; ao terminá-la, não é preciso refazer o checklist inteiro da etapa 24, só confirmar que nada que já passava deixou de passar (`mvn clean verify` no projeto inteiro continua sendo o teste definitivo disso) |
| `26_templates_de_produto_e_seed_de_esqueleto.md` | ADR-0017 + auditoria de jornadas (`docs/implementation/005_aegis_pms_user_journeys.md`) | `POST /products` ganha esqueleto de páginas por `type` (catálogo fechado, 7 valores incluindo `"Custom"`) e pré-habilitação de módulos recomendados; `PUT /tenants/{tenantId}` ganha notificação automática ao mudar status (suspender/reativar) — depende das etapas 07, 23 e 25 já existirem |
| `27_dominio_feedback.md` | Sprint 18 do frontend (botão "Reportar problema" sem backend) | Domínio `feedback`: persiste o bug-report do usuário, com anexo reaproveitando o upload de assets já existente (etapa 12), nunca um mecanismo de upload próprio — depende das etapas 06 e 12 |
| `28_backup_e_exportacao_na_exclusao.md` | ADR-0020 (ciclo de vida) | Backup completo em ZIP (JSON por domínio + assets físicos) disparado na exclusão de produto ou tenant; link de download por e-mail válido por 7 dias; `DELETE /products/{productId}` (novo); hook em `DELETE /tenants/{tenantId}` (etapa 10); template `productExport.ftl` — depende das etapas 06, 10, 11, 12, 13, 18, 23 |
| `29_tokens_de_ativacao_e_reset_de_senha.md` | Sprint 20 do frontend (`docs/sprints/frontend/20_ativacao_e_reset_de_senha_reais.md`) | Substitui `executeActionsEmail` do Keycloak por tokens gerenciados pelo Aegis: link de convite e reset aponta para o frontend (`/invite?token=` e `/reset-password?token=`); backend chama Keycloak Admin API para definir senha; novos endpoints públicos `/auth/invite/validate`, `/auth/activate`, `/auth/reset-password/request`, `/auth/reset-password/confirm`; entidade `AuthActionToken` com expiração automática; rate limiting nativo; templates `inviteActivation.ftl` e `passwordReset.ftl` com link Aegis — depende das etapas 06 e 07 |
| `30_seed_homologacao_e_remocao_seed_java.md` | **ETAPA FINAL** | Remove o package Java `seed` (`DemoSeedRunner`, `DemoSeedService`) e suas `@NamedInterface`s exclusivas; cria migration SQL idempotente com o tenant **CLIENTES BETA** + 6 produtos reais (Maestro Beton, Conecta Talentos, Alexandre Dev, CMSS, WikiDev, Loki) + suas páginas/seções + Knowledge Graphs (WikiDev, Loki, Maestro Beton); adiciona `GraphNodeType` TOPIC/POEM/MUSIC_REF/MANIFEST/BOOK/PLAYLIST; fecha retrofits de auditoria (module enable/disable, form submission, traceId/ip/userAgent via interceptor) — depende de todas as etapas anteriores |

Ao final da etapa 24, o backend completo deve estar funcional: `docker compose up -d` sobe tudo (incluindo MailHog), Keycloak e PostgreSQL persistem dados entre restarts, o backend autentica via JWT **e expõe proxy de auth** (login/logout/refresh sem o frontend conhecer o Keycloak), o modelo core e o Knowledge Graph MVP funcionam, **todos os domínios de produto que o frontend já usa (content, pages, assets, forms, analytics, users, audit, settings, dashboard) respondem com os payloads exatos do trace report**, o fluxo de Super Admin (criar tenant → criar produto → atribuir usuário) funciona de ponta a ponta via API, e o build do frontend React é servido na mesma origem. Ao final da etapa 25, além de tudo isso, o sistema de notificações (onboarding + direcionadas) também está funcional. Ao final da etapa 26, criar um produto já entrega o esqueleto de páginas certo para o tipo escolhido (ou nada, se `"Custom"`), e suspender/reativar um tenant avisa os usuários afetados. **Ao final da etapa 30 (última etapa de backend)**, o seed Java é removido, os dados reais de homologação do tenant CLIENTES BETA estão no banco via migration SQL idempotente (6 produtos + páginas + KG), e os retrofits de auditoria estão fechados — o backend está pronto para receber os convites dos usuários beta reais.
