...
- **Java 25** / **Spring Boot 4.1.x**. Javadoc obrigatório na interface e em todo método de `PageRepository`, `PageSectionRepository`, `ProductGlobalsRepository`. Mappers via MapStruct (`PageMapper`, `PageSectionMapper`, `ProductGlobalsMapper`). 100% de cobertura nas classes funcionais — incluindo `SectionContentValidationService` (a função `validateSectionContent` da Seção C, que é a classe mais crítica desta etapa: cada linha da tabela de validação precisa de pelo menos um teste de caminho feliz e um de rejeição) e o sanitizador de markdown reaproveitado da etapa 10.
- Entregar em rodadas:
  1. `Page`, `PageSection`, `ProductGlobals` (entities) + `PageRepository`, `PageSectionRepository`, `ProductGlobalsRepository` + testes `@DataJpaTest`.
  2. `PageMapper`, `PageSectionMapper`, `ProductGlobalsMapper` (MapStruct) + testes de mapper.
  3. `SectionContentValidationService` + `PageService` + `ProductGlobalsService` (regras das Seções C/E/F) + testes com mocks — cada `BlockType` da tabela da Seção C com pelo menos 2 testes (válido e inválido).
  4. `PageController`, `ProductGlobalsController` (endpoints da Seção D/F) + testes `@WebMvcTest` + validação via `curl`.
