...
A pasta `99-teardown` roda por último e remove somente dados marcados como teste. O step `02-limpar-usuarios-temporarios` remove memberships e usuários Keycloak com e-mails temporários conhecidos (`guest-*`, `tenant-user-*`, `notification-user-*`, `auth-action-*`, `bruno-rbac-*`, `matrix-*`). O step `03-limpar-tenants-de-teste` remove tenants marcados com prefixo `TEST-`, aplicado na criação dos tenants pela própria collection, e também reconhece resíduos antigos de testes locais/Maven (`Tenant listener-commit`, `Tenant listener-rollback`, `Tenant <uuid>`, `Tenant E2E <timestamp>`, `TEST-Maven ...` e `TEST-Form Listener ...`), porque esses nomes existiram antes da padronização.
...
- **Bug 7 (`32-role-permissions`)**: `rbacTenantId`, `rbacProductId`, `rbacSuperToken`, `rbacPmToken`, `rbacEditorToken`, `rbacViewerToken`, `brunoRbacProductInviteEmail`, `brunoRbacProductUserId`, `brunoRbacTenantInviteEmail`, `brunoRbacTenantUserId` e `brunoRbacDeniedInviteEmail`. A pasta valida as permissões vivas dos papéis seedados em `CLIENTES BETA`/`Maestro Beton`: Product Manager lista e convida membro no produto onde é PM; Editor não convida; Viewer não remove; Product Manager só convida pelo endpoint de tenant quando limita o convite aos produtos que gerencia. O teardown 99 remove automaticamente os usuários temporários criados.
