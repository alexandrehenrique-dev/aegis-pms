<!DOCTYPE html>
<html lang="pt-BR">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width,initial-scale=1.0">
  <title>Você foi convidado — Aegis PMS</title>
</head>
<body style="margin:0;padding:0;background-color:#f5f5f7;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Inter',sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#f5f5f7;padding:40px 16px;">
    <tr>
      <td align="center">
        <table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08);">

          <!-- ── Header ── -->
          <tr>
            <td align="center" style="background-color:#7c3aed;padding:22px 40px;">
              <table align="center" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:50px;">
                <tr>
                  <td style="padding:10px 14px 10px 14px;vertical-align:middle;">
                    <img src="data:image/png;base64," width="40" height="40" alt="Aegis PMS" style="display:block;border:0;border-radius:6px;">
                  </td>
                  <td style="padding:10px 20px 10px 2px;vertical-align:middle;font-size:17px;font-weight:700;color:#1d1d1f;letter-spacing:-0.4px;white-space:nowrap;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Inter',sans-serif;">
                    Aegis <span style="color:#7c3aed;">PMS</span>
                  </td>
                </tr>
              </table>
            </td>
          </tr>
          <!-- ── Body ── -->
          <tr>
            <td style="padding:36px 40px 28px;color:#374151;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Inter',sans-serif;">
              <p style="margin:0 0 16px;font-size:15px;font-weight:400;color:#374151;line-height:1.65;">Olá, <strong>${userName}</strong>.</p>
              <p style="margin:0 0 16px;font-size:15px;font-weight:400;color:#374151;line-height:1.65;"><strong>${inviterName}</strong> convidou você para trabalhar no <strong>Aegis PMS</strong>. Clique no botão abaixo para definir sua senha e ativar sua conta.</p>
              <!-- Invite detail card -->
              <table cellpadding="0" cellspacing="0" width="100%" style="margin:20px 0;background-color:#f0ebff;border:1px solid #ddd6fe;border-radius:10px;">
                <tr>
                  <td style="padding:16px 20px;">
                    <p style="margin:0 0 12px;font-size:11px;font-weight:700;color:#7c3aed;text-transform:uppercase;letter-spacing:0.9px;">Detalhes do convite</p>
                    <table cellpadding="0" cellspacing="0" width="100%">
                      <tr>
                        <td style="padding:5px 0;font-size:13px;color:#6e6e73;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Inter',sans-serif;">Workspace</td>
                        <td style="padding:5px 0;font-size:13px;font-weight:600;color:#1d1d1f;text-align:right;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Inter',sans-serif;">${tenantName}</td>
                      </tr>
                      <tr>
                        <td style="padding:5px 0;font-size:13px;color:#6e6e73;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Inter',sans-serif;">Papel</td>
                        <td style="padding:5px 0;font-size:13px;font-weight:600;color:#1d1d1f;text-align:right;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Inter',sans-serif;">${role}</td>
                      </tr>
                      <tr>
                        <td style="padding:5px 0;font-size:13px;color:#6e6e73;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Inter',sans-serif;">Produtos</td>
                        <td style="padding:5px 0;font-size:13px;font-weight:600;color:#1d1d1f;text-align:right;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Inter',sans-serif;">${productNames}</td>
                      </tr>
                    </table>
                  </td>
                </tr>
              </table>
              <!-- CTA -->
              <table cellpadding="0" cellspacing="0" style="margin:24px 0;">
                <tr>
                  <td style="background-color:#7c3aed;border-radius:8px;mso-padding-alt:0 32px;">
                    <a href="${activationUrl}" style="display:inline-block;padding:14px 32px;color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;letter-spacing:0.1px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Inter',sans-serif;">Ativar minha conta</a>
                  </td>
                </tr>
              </table>
              <p style="margin:6px 0 20px;font-size:12px;color:#9ca3af;text-align:center;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Inter',sans-serif;">
                Este link expira em <strong>${expiresAt}</strong>.
              </p>
              <!-- Warning -->
              <table cellpadding="0" cellspacing="0" width="100%" style="margin:8px 0 0;">
                <tr>
                  <td style="background-color:#fffbeb;border:1px solid #fcd34d;border-radius:8px;padding:12px 16px;font-size:13px;color:#92400e;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Inter',sans-serif;line-height:1.5;">
                    Se você não esperava este convite, ignore este e-mail. Nenhuma ação é necessária.
                  </td>
                </tr>
              </table>
            </td>
          </tr>
          <!-- ── Footer ── -->
          <tr>
            <td style="background-color:#f5f5f7;border-top:1px solid #e5e7eb;padding:18px 40px;text-align:center;">
              <p style="margin:0;font-size:12px;color:#6e6e73;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Inter',sans-serif;">
                Aegis PMS · Sistema de Gestão de Produtos
              </p>
              <p style="margin:6px 0 0;font-size:11px;color:#9ca3af;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Inter',sans-serif;">
                Você recebeu este e-mail porque foi convidado para a plataforma Aegis PMS.
              </p>
            </td>
          </tr>
        </table>
      </td>
    </tr>
  </table>
</body>
</html>