...
export const productAssignmentsService = {
  ...
  async assign(req: AssignProductUserRequest): Promise<ProductAssignmentSummary> {
    ...
    if (req.inviteEmail) {
      const invited = await usersService.invite({ name: req.inviteName || req.inviteEmail, email: req.inviteEmail, role: req.role, allowedProducts: product?.name ?? req.productId });
