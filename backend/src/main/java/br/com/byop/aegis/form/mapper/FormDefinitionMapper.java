...
public interface FormDefinitionMapper {
    ...
    String NO_ACTIVITY_PLACEHOLDER = "—";
    String NO_PUBLICATION_PLACEHOLDER = "—";

    @Mapping(target = "status", expression = "java(form == null ? null : toContractStatus(form.getStatus()))")
    @Mapping(target = "responses", expression = "java(toResponses(responses))")
    @Mapping(target = "conversion", source = "conversion")
    @Mapping(target = "lastActivity", expression = "java(toLastActivity(lastActivity))")
    @Mapping(target = "publication", expression = "java(form == null ? null : toPublication(form.getPublication()))")
    FormSummary toSummary(FormDefinition form, long responses, String conversion, OffsetDateTime lastActivity);
    ...
    default String toLastActivity(OffsetDateTime lastActivity) {
        return lastActivity == null ? NO_ACTIVITY_PLACEHOLDER : lastActivity.toString();
    }
